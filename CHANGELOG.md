...
### Bugfixes:
- All PhotoServers:
  - Fixed the keyword, person and location tag comparison on metadata upload: tags of one type could leak into the tag list of another type, so the wrong tags were added to or removed from a photo
  - Fixed the 'Sort Photos' option of a Published Collection being ignored, so a custom sort order was never applied at the PhotoServer. A failing sort is now reported instead of being silently dropped
  - Fixed unrotated videos being hard-rotated by 180 degrees, as ffmpeg reports them as 'displaymatrix: rotation of -0.00 degrees' (follow-up to the displaymatrix support added in 7.5.0, see issue #81)
  - Fixed the program path checks in the Plugin Manager settings, which reported an error for a correct path to convert, dcraw or exiftool, and accepted a wrong one
- All PhotoServers / MacOS:
  - Fixed the default program paths on Apple Silicon Macs: convert, dcraw, exiftool and ffmpeg are now also looked up below '/opt/homebrew'. The non-symlinked '&lt;prefix&gt;/opt/&lt;formula&gt;/bin/&lt;tool&gt;' location is preferred, as symbolic links are not accepted as program paths and a versioned Cellar path does not survive a 'brew upgrade'
- All PhotoServers / Windows:
  - Fixed the quoting of the dcraw and ImageMagick command lines used for thumbnail generation: the closing quote was missing
- Photo Station:
  - Fixed an exception `attempt to index a nil value` when deleting a photo which does not exist on the PhotoServer, which happens before every video upload
  - Fixed photos not being removed from a Shared Album: the photo ids were generated by the Photos implementation and were therefore always empty
  - Fixed the private and public Shared Album URLs never being determined, so no Shared Album URL was ever stored in the plugin preferences
  - Fixed a bogus `interrupts upload of` error message being logged for every single uploaded file
- Photos:
  - Fixed the cleanup of empty folders deleting a folder whose contents could not be listed: a listing error, e.g. a session timeout, made a non-empty folder look empty and it was deleted together with its parents
